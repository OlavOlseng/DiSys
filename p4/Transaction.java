...
class Transaction {
	...
	private synchronized boolean acquireLock(ResourceAccess resourceAccess) {
		...
		do {
			...
			if (Globals.PROBING_ENABLED) {
				...
				this.owner.fireProbe(transactions,
						waitingForResource.resourceId,
						waitingForResource.server);

				try {
					wait(Globals.TIMEOUT_INTERVAL / 2);
				} catch (InterruptedException e) {
					e.printStackTrace();
				}
			}
		} while (!abortTransaction && Globals.PROBING_ENABLED);
	...
	public synchronized void receiveProbe(ArrayList<Integer> transactions) {
		...
		transactions.add(this.transactionId);
		this.owner.fireProbe(transactions, waitingForResource.resourceId,
				waitingForResource.server);
	...
	private synchronized void abort() {
		...
		owner.println("Aborting transaction " + transactionId + '.',
				transactionId);
		System.err.println("Aborting transaction " + transactionId + '.'
				+ transactionId);
		releaseLocks();
